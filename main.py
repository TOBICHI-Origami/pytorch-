...
def train(train_data_loader , eval_data_loader, model ,optimizer ,num_epoch,log_step_interval, save_step_interval,eval_step_interval,save_path,resume=' '):
    ...
    for epoch_index in range(start_epoch, num_epoch):
        ...
        for batch_index,(target,token_index) in enumerate(train_data_loader):
            ...
            bce_loss = F.binary_cross_entropy(torch.sigmoid(logits),F.one_hot(target,num_classes=2).to(torch.float32)).to(device)
            ...
            if step% eval_step_interval==0:
                ...
                for eval_batch_index,(eval_target,eval_token_index) in enumerate(eval_data_loader):
                    ...
                    eval_bce_loss = F.binary_cross_entropy(torch.sigmoid(eval_logits),F.one_hot(eval_target,num_classes=2).to(torch.float32)).to(device)
                    ema_eval_loss = 0.9*ema_eval_loss+0.1*eval_bce_loss
                print("accuracy percentage is {}".format(total_acc_acount/(total_account)))
                model.train()
def ceshi(a,resume):
        print("logging from {}".format(resume))
        checkpoint = torch.load(resume)
        ...
        start_step = checkpoint['step']
        target = [0]
        target = torch.tensor(target).to(torch.int64).to(device)
        token_index = []
        max_len=256
        tokens = tokenizer(a)
        token_index.append(vocab(tokens))
        token_index = [index+[0]*(max_len-len(index))for index in token_index]
        b=(torch.tensor(token_index).to(torch.int32))
        b=b.to(device)
        eval_logits = model(b)
        print(eval_logits)
        return "positive" if(torch.argmax(eval_logits,dim=-1)==target) else "negative"
...
if __name__ == '__main__':
    ...
    #测试
    resume = './logs_imdb_text_classification/step3500.pt'
    a=("i think it is very bad")
    b=("it look like a beautiful boy i think it is good")
    
    print(ceshi(a,resume))
    print(ceshi(b,resume))
